...
fn count_page(lmti: usize, pg: &[u32]) -> i64 {
    ...
    let (lmt, icnt) = if lmti >= pgbts {
        ...
        (
            lstw,
            (CLUT[v & 0xFF] + CLUT[(v >> 8) & 0xFF] + CLUT[(v >> 16) & 0xFF] + CLUT[v >> 24])
                as u32,
        )
    };
    let mut count = 0u32;
    for i in 0..lmt {
        let v = pg[i] as usize;
        count += (CLUT[v & 0xFF] + CLUT[(v >> 8) & 0xFF] + CLUT[(v >> 16) & 0xFF] + CLUT[v >> 24])
            as u32;
    }
    (icnt + count) as i64
}

fn primes_pages() -> Box<dyn Iterator<Item = (u64, Vec<u32>)>> {
    // a memoized iterable enclosing a Vec that grows as needed from an Iterator...
    type Bpasi = Box<dyn Iterator<Item = (u64, Vec<u32>)>>; // (lwi, base cmpsts page)
    ...
    fn make_page(lwi: u64, szbts: u64, bppgs: &Bpas) -> (u64, Vec<u32>) {
        let nxti = lwi + szbts;
        let pbts = szbts as usize;
        let mut cmpsts = vec![0u32; pbts / 32];
        'outer: for bpg in Bps(bppgs.clone()).into_iter() {
            // in the inner tight loop...
            let pgsz = bpg.len();
            for i in 0..pgsz {
                let p = bpg[i] as u64;
                let pc = p as usize;
    ...
    fn pages_from(lwi: u64, szbts: u64, bpas: Bpas) -> Box<dyn Iterator<Item = (u64, Vec<u32>)>> {
        struct Gen(u64, u64);
        impl Iterator for Gen {
            ...
            fn next(&mut self) -> Option<(u64, u64)> {
                let v = self.0;
                let inc = self.1; // calculate variable size here
                self.0 = v + inc;
                Some((v, inc))
            }
        }
        Box::new(Gen(lwi, szbts).map(move |(lwi, szbts)| make_page(lwi, szbts, &bpas)))
    }
    fn cnvrt2bppg(cmpsts: (u64, Vec<u32>)) -> Vec<u32> {
        ...
        let mut j = 0;
        let bsp = (lwi + lwi + 3) as usize;
        for i in 0..pgbts {
            if (pg[i >> 5] & (1u32 << (i & 0x1F))) == 0u32 {
                bpv[j] = (bsp + i + i) as u32;
                j += 1;
            }
        }
        bpv
...
fn primes_paged() -> Box<dyn Iterator<Item = u64>> {
    fn list_paged_primes(
        cmpstpgs: Box<dyn Iterator<Item = (u64, Vec<u32>)>>,
