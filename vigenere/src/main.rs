...
impl Vigenere { 
 ...
 fn new(key:&'static str,alphabet:& 'static str) -> Vigenere { 
       ...
       Vigenere { 
           key,alphabet:alphabet.chars().collect(), shifts:vec![]
       }
 }
 #[allow(dead_code)]
 #[inline(always)]
 ...
 let v:Vec<char> = self.key.chars().collect();
  let repeats:Vec<char>= msg.chars().collect::<Vec<char>>().chunks(self.key.chars().count()).map(|e|{ 
e.iter().enumerate().map(|(i,_n)| v[i])


  }).flatten().collect();
  // print repeats
 
  self.shifts = repeats.iter().map( |e| self.alphabet.iter().position(|n|n==e).unwrap()).collect();
  // print 
   
   msg.char_indices().map(|(ind,c)|{ 
...
msg.char_indices().map(|(ind,c)|{ 
    ...
    return self.alphabet[(pos+self.alphabet.len()-self.shifts[ind])%self.alphabet.len()]
    }
    c
