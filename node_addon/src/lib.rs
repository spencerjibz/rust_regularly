...
#[napi]
pub fn long_addition(right:String,left:String) -> String { 
...
let mut result:Vec<char> = longest.clone();
 ...
 let mut remainder =0;
 for (ind,d) in longest.iter().enumerate() { 
       let sum = (remainder + d + shortest[ind]).to_string();
