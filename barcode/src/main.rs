...
fn read_barcode(first: usize, code: &str) -> String {
    ...
    let bin_nums = first_half
        .as_bytes()
        .chunks(7)
        .map(str::from_utf8)
        .collect::<Result<Vec<&str>, _>>()
        .unwrap();
    let last_numbs = second_half
        .as_bytes()
        .chunks(7)
        .map(str::from_utf8)
        .collect::<Result<Vec<&str>, _>>()
    ...
    let mut encod: Vec<(String, String)> = word
        .chars()
        .enumerate()
        .map(|(i, e)| (format!("{}{}", e, i), bin_nums[i].to_string()))
        .collect();
    ...
    let last: String = last_numbs
        .iter()
        .map(|e| RCODE.iter().position(|f| f == e).unwrap().to_string())
        .collect();
...
fn encode_barcode(code: &str) -> String {
    let barcode = if code.len() <= 12 {
        format!("{}{}", code, check(code))
    } else {
        code.to_string()
    };

    let (first, first_six, last) = (
        barcode[..=0].parse::<usize>().unwrap(),
        &barcode[1..=6],
        &barcode[7..],
    );
    ...
    let last_six = last
        .chars()
        .map(|e| RCODE[e.to_digit(10).unwrap() as usize]);

    format!(
        "101{}01010{}101",
        first_six_str.collect::<String>(),
        last_six.collect::<String>()
    )
}
#[allow(dead_code, unused_variables, unused_imports)]
fn check(code: &str) -> u32 {
    let digits = code
        ...
        .map(|e| e.to_digit(10u32).unwrap());
    // get some
    let pairs = digits.enumerate().map(|(i, e)| (i + 1, e));

    let sum = pairs
        .map(|(ind, n)| if ind % 2 != 0 { n * 1 } else { n * 3 })
        .reduce(|c, n| c + n)
        .unwrap()
        * 1;
...
fn main() {
   ...
   println!("{}", read_barcode(5, code));
      println !("{}",encode_barcode("5901234123457") ==code);
