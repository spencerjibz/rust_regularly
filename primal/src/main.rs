...
use num::{bigint::BigUint, One,integer::Integer,iter};
...
#[inline(always)]
   fn sum_fracts(l: Vec<(i64, i64)>) -> Option<(i64, i64)> {
    // your code
    let lc = l.clone().into_iter().map(|n|n.1).skip(1).fold(l[0].1,|c,n|(c).lcm(&n));
  // 
   let division = l.into_iter().map(|c|(lc/c.1)*c.0).fold(0,|cu,ne|cu+ne);
...
#[inline(always)]
  fn convert_fracts(l: Vec<(i64, i64)>) -> Vec<(i64, i64)> {
    ...
    let lc = l.clone().into_iter().map(|n|n.1).skip(1).fold(l[0].1,|c,n|(c).lcm(&n));
    l.into_iter().map(|x|((lc/x.1)*x.0,lc)).map(|n|{
