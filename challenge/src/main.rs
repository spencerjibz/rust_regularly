...
fn big_fact(n:i32) -> BigUint { 
    return (1..=n).map(|e|BigUint::from(e as u32)).reduce(|c,n|c*n).unwrap()
}
#[allow(dead_code)]
...
if alph.contains(source) { // move to different register
...
//println!("{}",len_bit);
s.as_bytes().iter().for_each(|e| bits.push_str(&format!("{:08b}",e)));

let  mut bits:String = bits.chars().map(|e|{
...
let groups:Vec<usize> = sub_strings(&bits, 8).iter().map(|e|usize::from_str_radix(e,2).unwrap()).collect();
//println!("{:?}",groups);

// message Polynomial - ax^25 + bx^24+ cx^23 + dx^22 + ex^21+ Fx^20 = gx^19 + hx^18 + ix^17
let  mut message:Vec<usize> = (17usize..=25).rev().enumerate().map(|(ind,_)|groups[ind]).collect();

// generator polynomial  = (alphapower)
let g = vec![0,43,139,206,78,43,239,123,206,214,147,24,99,150,39,243,163,136];

let g_field = [1,2,4,8,16,32,64,128,29,58,116,232,205,135,19,38,76,152,45,90,180,117,234,201,143,3,6,12,24,48,96,192,157,39,78,156,37,74,148,53,106,212,181,119,238,193,159,35,70,140,5,10,20,40,80,160,93,186,105,210,185,111,222,161,95,190,97,194,153,47,94,188,101,202,137,15,30,60,120,240,253,231,211,187,107,214,177,127,254,225,223,163,91,182,113,226,217,175,67,134,17,34,68,136,13,26,52,104,208,189,103,206,129,31,62,124,248,237,199,147,59,118,236,197,151,51,102,204,133,23,46,92,184,109,218,169,79,158,33,66,132,21,42,84,168,77,154,41,82,164,85,170,73,146,57,114,228,213,183,115,230,209,191,99,198,145,63,126,252,229,215,179,123,246,241,255,227,219,171,75,150,49,98,196,149,55,110,220,165,87,174,65,130,25,50,100,200,141,7,14,28,56,112,224,221,167,83,166,81,162,89,178,121,242,249,239,195,155,43,86,172,69,138,9,18,36,72,144,61,122,244,245,247,243,251,235,203,139,11,22,44,88,176,125,250,233,207,131,27,54,108,216,173,71,142,1];
// 
...
//println!("prev ={:?}", Firstind);
 ...
 //println!("prev ={:?}",poly);
 message = poly.enumerate().map(|(ind,e)|{
// find the  iterator corresponding to postion
let int =   if e < 254 {g_field[e]} else { g_field[e%255]};
...
fn fill_area( matrix:&mut Vec<Vec<usize>>,row:usize,col:usize, width:usize,height:usize,fill:usize) { 
...
for exponent in 1..256 { 
  value =  if value > 127 {(value<<1)^285} else {value<<1};
 ...
 for _ in 0.. quotientLength { 
    ...
    let ans = polyMul(divisor.clone(),vec![factor]);
     for i in 0..=ans.len()-1 { 
      ...
      rest = rest.into_iter().enumerate().map(|(index,value)| value^subtr[index]).collect::<Vec<usize>>()[1..].to_vec();
...
let mut count =0;
 for i in 5..=5+ rest.len()-1 { 
  ...
  formatPoly.iter().enumerate().map(|(index,bit)| bit^FORMAT_MASK[index]).collect()
...
#[allow(dead_code,non_snake_case)]
fn get_masked_qr(version:usize,codewars:Vec<usize>,error_level:char,maskIndex:usize) -> Vec<Vec<usize>> { 
...
format_modules[0..7].iter().enumerate().for_each(|(index,cell)| { 
 matrix[length-index-1][8] = *cell;
});
format_modules[..=9].iter().enumerate().for_each(|(index,cell)| { 
...
[[0,0],[size-7,0],[0,size-7]].iter().for_each(|[row,col]|{ 
...
// get the codewars
let codewars:Vec<usize>= sub_strings(&message, bit_length).iter().map(|e|usize::from_str_radix(e,2).unwrap()).collect();
...
fn hamming(n:usize) -> usize {
 ...
 let x = *[left,next,right].iter().reduce(|a,b| std::cmp::min(a,b)).unwrap();
 seq.push(x);
 if left <=x { 
   i2 +=1;
 }
...
#[inline]
 pub fn get_shifted_str_right(sum:&str,right:usize,to_right:bool) -> String { 
  ...
  // get an ar
   ...
   let  right =  if right > arr.len() {right%arr.len()} else {right};
   if to_right { 
...
pub fn encode(n:u32,qoute:&str,) -> String { 
 let mut message = if qoute.contains('"') {qoute.escape_default().to_string()} else { qoute.to_owned()};

 let spaces:Vec<(usize,char)> = message.char_indices().filter(|(_,c)|c==&' ').collect();
 // get no_space
...
let rotate = if n as usize > no_spaces.len() {n as usize%no_spaces.len()} else {n as usize};
 no_spaces.rotate_right(rotate);
 // place back the spaces in their original spaces
 spaces.iter().for_each(|(ind,s)| { 
 ...
 // 
  ...
  message =  after.split(" ").map(|e| get_shifted_str_right(e,rotate,true)).join(" ");
   //println!("{}",message);
...
let mut count =1;
while count <=n {
  message = message.split(' ').map(|e| get_shifted_str_right(e,n,false)).join(" ");
